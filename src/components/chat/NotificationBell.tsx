...
export function NotificationBell() {
  ...
  const { ideaNotifications, ideaUnreadTotal, markIdeaRead } = useChatNotifications()
  ...
  return (
    <div className="relative" ref={panelRef}>
      <button
        ...
        aria-label="התראות צ'אט רעיונות"
        aria-expanded={open}
      >
        ...
        {ideaUnreadTotal > 0 && (
          <span className="absolute -left-0.5 -top-0.5 flex h-5 min-w-5 items-center justify-center rounded-full bg-error px-1 text-[10px] font-bold text-white">
            {ideaUnreadTotal > 9 ? '9+' : ideaUnreadTotal}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-2 w-[min(90vw,20rem)] animate-fade-up rounded-2xl border border-border-light bg-surface-container-lowest/95 p-2 shadow-boutique backdrop-blur-xl">
          <p className="px-3 py-2 font-label-md text-on-surface">התראות צ'אט רעיון</p>
          {ideaNotifications.length === 0 ? (
            <p className="px-3 py-6 text-center font-label-sm text-secondary">
              אין הודעות חדשות ברעיונות שלך
            </p>
          ) : (
            <ul className="max-h-72 space-y-1 overflow-y-auto">
              {ideaNotifications.map((n) => (
                <li key={n.ideaId}>
                  <button
                    type="button"
                    onClick={() => handleOpenIdea(n.ideaId)}
                    className="flex w-full gap-3 rounded-xl px-3 py-3 text-right transition-colors hover:bg-primary/10"
                  >
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-inbox/15 text-inbox">
